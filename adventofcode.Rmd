...
expand_grid(zahlen3, zahlen4, .name_repair = "minimal") %>% 
  mutate(summe = zahlen3 + zahlen4) %>% 
  filter(summe == 2020) %>% 
  mutate(product = zahlen3 * zahlen4)
```

Stern 2

```{r}
expand_grid(x = zahlen3, y = zahlen3, z = zahlen3) %>% 
  mutate(summe = x + y + z) %>% 
  filter(summe == 2020) %>% 
